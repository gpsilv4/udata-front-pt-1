from flask import abort, request, url_for, make_response, current_app
...
def recent_feed():
    ...
    reuses = (
        Reuse.objects.visible().order_by("-created_at").limit(current_site.feed_size)
    )
    for reuse in reuses:
        author_name = None
        author_uri = None
        if reuse.organization:
            ...
            author_uri = url_for(
                "organizations.show", org=reuse.organization.id, _external=True
            )
        elif reuse.owner:
            author_name = reuse.owner.fullname
            author_uri = url_for("users.show", user=reuse.owner.id, _external=True)
        if not author_name:
            author_name = current_site.author or current_app.config.get("SITE_AUTHOR")
...
class ReuseDetailView(ReuseView, DetailView):
    ...
    def get_context(self):
        ...
        followers = Follow.objects.followers(self.reuse).order_by("follower.fullname")
